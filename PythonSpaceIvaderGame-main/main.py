"""
1. Polymorphism - def move_lasers
2. Inheritance - class Player(Ship)  &  class Enemy(Ship)
3. Encapsulation -  __ROCKY_MAP  &  __COLOR_MAP
"""
...
def main():
    ...
    def redraw_window():
        ...
        if lost:
            lost_label = lost_font.render("Game Over", 1, (255, 255, 255)) # Text when player lost
    ...
    while run:
        ...
        if lost:
            if lost_count > FPS * 3:
                ...
                file.write(f"Destroyed enemy ships: {score/10} " + f"Level: {level} " + "Date:" +datetime.today().strftime('%Y-%m-%d %H:%M:%S') + "\n")
                file.close
                file = open ("scores.txt")
                pattern = re.compile("Destroyed enemy ships:")
                find = 0
                for line in file:
                ...
                if find > 0:
                    file.close
                    file = open ("scores.txt", "a+") 
                    file.write("Your score has been saved\n")  
                    file.close  
                run = False 
            else: 
                continue
